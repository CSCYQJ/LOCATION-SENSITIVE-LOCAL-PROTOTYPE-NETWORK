...
class FewShotSeg(nn.Module):
    ...
    def forward(self, supp_imgs, fore_mask, back_mask, qry_imgs):
        ...
        for epi in range(batch_size):
            ...
            if self.config['align'] and self.training:
                ...
                dualLoss=self.dualLoss(prototypes,supp_fts[:, :, epi],
                                                fore_mask[:, :, epi], back_mask[:, :, epi])
                align_loss += (align_loss_epi+dualLoss)

        output = torch.stack(outputs, dim=1)  # N x B x (1 + Wa) x H x W
        output = output.view(-1, *output.shape[2:])
        return output, align_loss / batch_size,supp_fts,qry_fts
    ...
    def dualLoss(self,prototypes,supp_fts,fore_mask,back_mask):
        """
        Compute the loss for the prototype dual branch

        Args:
            prototypes: [fg,bg] prototypes for support images
                expect shape: 1 x C 
